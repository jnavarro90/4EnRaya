...
public class RecursosAppSwing {
  ...
  * instancia
  */        
  public static synchronized RecursosAppSwing instancia(
          String fichRecursos) {
    if (instancia == null)
  ...
  */    
  private void cargaMenus() {
    List<String> menusID = recursos.getValoresElementoEtiqueta(
            MENU, ID);
    List<String> menusTexto = recursos.getValoresElementoEtiqueta(
            MENU, TEXTO);
  ...
  private void cargaMenusItem() {
    ...
    for(int i = 0; i < menusItemId.size(); i++)
      menusItem.put(menusItemId.get(i), 
                    new Menu(menusItemTexto.get(i), 
                            menusItemAtajo.get(i)));      
  }

 ...
 * modelo, se encarga de poner las fichas en el TableroVista.java 
 * y contiene lo que vera el usuario para jugar.
 */
package vista;
...
public class TableroSwing extends JFrame 
    ...
    private static final String MENU_ITEM_ACERCA_DE = ""
            + "MENU_ITEM_ACERCA_DE";
    ...
    TableroSwing(int numFilTab, int numColTab,
        ...
        setSize((int) (tableroVista.dimensionCasilla().getWidth()
                * numColTab + ANCHURA_LISTA_JUGADORES + 
                MARGEN_HORIZONTAL),
        ...
        for (int fil = 1; fil < MAX_FILAS; fil++) {
            for (int col = 0; col < MAX_COLUMNAS; col++) {
                if (casillas[fil][col].getSimbolo().equals("#")) {
                    tableroVista.ponerIconoCasilla(invertirNumero(fil),
                            col, recSwingApp.getIconoColor1());
                } else if (casillas[fil][col].getSimbolo().equals("o")) {
                    tableroVista.ponerIconoCasilla(invertirNumero(fil), 
                            col, recSwingApp.getIconoColor2());
                }
            }
        }
    }
